...
(function () {
  ...
  function buildCampfire(scene, cx, cz, baseY) {
    ...
    const fireLayers = [
      { mesh: null, w: 0.52, h: 0.78, y: 0.22, x: 0, phase: 0 },
      { mesh: null, w: 0.38, h: 0.58, y: 0.28, x: 0.04, phase: 1.4 },
      { mesh: null, w: 0.26, h: 0.42, y: 0.34, x: -0.03, phase: 2.6 },
    ];
    ...
    const light = new THREE.PointLight(0xff6620, 3.5, 32, 1.5);
    light.position.set(0, 0.55, 0);
    root.add(light);

    const fill = new THREE.PointLight(0xffaa44, 0.5, 14, 2);
    ...
    if (ZS.registerFireLight) {
      ZS.registerFireLight(light, fireLayers[0].mesh, {
        ...
        onTick(t, flicker, night) {
          ...
          if (fill) fill.intensity = (0.22 + flicker * 0.1) * (0.5 + (night || 0) * 0.8);
        },
      });
    }
