// Sentiers — refonte spawn : ruban terrain + aplatissement, sans RoadNetwork legacy.
...
(function () {
  ...
  /** Ruban texturé qui suit le terrain — échantillonnage continu, pas de triangles fantômes. */
  ...
  function buildMesh(scene, pts, opts) {
    ...
    for (const sample of samples) {
      ...
      for (let c = 0; c <= COLS; c++) {
        ...
        const py = ZS.getDecorGroundHeight
          ? ZS.getDecorGroundHeight(px, pz)
          : (ZS.getTerrainHeight ? ZS.getTerrainHeight(px, pz) : 0) + LIFT;
