...
(function () {
  ...
  function isNearRoad(x, z, margin) {
    ...
    const m = margin || 0;
    for (const c of _clearings) {
      const pad = m / Math.min(c.rx, c.rz);
      if (_normClearingDist(x, z, c) <= 1 + pad) return true;
    }
    for (const e of _resolved) {
  ...
  function buildMeshes(scene, M) {
    ...
    for (const c of _clearings) {
      ...
      let cone = null;
      for (const e of _resolved) {
        if (_findLinkedClearing(e) === c) { cone = edgeExits.get(e.id); break; }
      }
      _buildClearingDisc(scene, c, clearMat, cone);
    }
