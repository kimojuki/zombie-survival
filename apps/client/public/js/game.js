...
(async function () {
  ...
  function _updateDoorInteractUi() {
    ...
    if (!storage && !door && !_nearDoor && !_nearStorage) return;
    _nearStorage = storage;
    _nearDoor = door;
    const btn = _ensureDoorButton();
    if ((!storage && !door) || state.player.dead || ZS.Rcon?.isOpen?.() || ZS.Chat?.isOpen?.()) {
      btn.style.display = 'none';
      return;
    }
    const action = storage ? 'Coffre' : (door.open ? 'Fermer' : 'Ouvrir');
